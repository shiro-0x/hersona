...
`--strict` / `--check-prompt` (measure), `--memory` / `--memory-file` (soul/persistent),
`--format openai_assistants|langchain_system_message` (export) の具体例は
[REFERENCE.md](./REFERENCE.md#v140-追加フラグ詳細) を参照。`--lang {en,ja}` で出力言語を切替
(`HERSONA_LANG` 環境変数でも可)。`--plain` で rich テーブルを無効化。
...
モード別の検証チェックリスト、4 モードを順に試す手順、recommend / export / measure /
soul / presets / 属性追加 / シェル補完などのコマンドレシピは
[REFERENCE.md](./REFERENCE.md) に分離 (会話時に毎ターン読み込まないための分割)。
必要になったら参照する。
...
- 詳細リファレンス (フラグ詳細 / Verification Checklist / One-Shot Recipes / バージョン履歴):
  [REFERENCE.md](./REFERENCE.md)

## Versioning

hersona / SKILL.md のバージョン履歴・廃止済みデータ形式・破壊的変更の一覧は
[REFERENCE.md](./REFERENCE.md#versioning) を参照。現行 SKILL は **v0.3.0** (詳細リファレンスを
REFERENCE.md へ分離し本体をスリム化)。
