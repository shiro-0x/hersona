...
**汎用属性テンプレート** (personality / speech / archetype の 38 種) を、
...
- 利用可能な 38 属性を確認したい (`/hersona list`)
...
# または
python3 scripts/validate.py  # 38 属性 YAML 自体のスキーマ整合確認
```
...
- [ ] `python scripts/validate.py` が 38 属性 / 0 エラーで exit 0
- [ ] `pytest` が全件パス (38 属性のスキーマ整合 / ファイル名一致 / カテゴリ一致)
...
- 38 属性テンプレート: `~/projects/hersona/attributes/`
