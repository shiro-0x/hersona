...
def test_all_attributes_present() -> None:
    """92 属性 (personality 42 / speech 31 / archetype 9 / visual 5 / hobby 5)。

    speech 31 = ja 25 + en 5 + archaic_otaku (Phase 8)。
    personality 42 = ja-base 35 + en-native 5 + hautaine + sociable (Phase 8)。
    """
    paths = _all_attribute_paths()
    names = [p.stem for p in paths]
    assert len(names) == 92, f"92 属性あるはずだが {len(names)} 件: {names}"
    ...
    assert len(by_cat["speech"]) == 31, by_cat
