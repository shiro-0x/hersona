...
- 92 属性 (personality 42 / speech 31 / archetype 9 / visual 5 / hobby 5) 揃っている
...
def test_all_attributes_present() -> None:
    """128 属性 (personality 42 / speech 67 / archetype 9 / visual 5 / hobby 5)。

    speech 67 = ja 25 + en 5 + archaic_otaku (Phase 8) + 36 regional dialects (Phase 1)。
    personality 42 = ja-base 35 + en-native 5 + hautaine + sociable (Phase 8)。
    """
    paths = _all_attribute_paths()
    names = [p.stem for p in paths]
    assert len(names) == 128, f"128 属性あるはずだが {len(names)} 件: {names}"
    ...
    assert len(by_cat["speech"]) == 67, by_cat
