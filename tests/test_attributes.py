...
- 83 属性 (personality 34 / speech 30 / archetype 9 / visual 5 / hobby 5) 揃っている
...
def test_all_attributes_present() -> None:
    """83 属性 (personality 34 / speech 30 / archetype 9 / visual 5 / hobby 5)。

    speech 30 = ja 25 + en 5 (Phase 5: formal/casual/blunt/southern_us/british _en)。
    """
    paths = _all_attribute_paths()
    names = [p.stem for p in paths]
    assert len(names) == 83, f"83 属性あるはずだが {len(names)} 件: {names}"
    ...
    assert len(by_cat["personality"]) == 34, by_cat
