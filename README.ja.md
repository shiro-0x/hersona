...
その効果の実測 (2026-07-12、minimax/MiniMax-M3、`tsundere + keigo` を
`--weight strong` で、人格上書き攻撃シナリオ):
...
追従できません。hersona は同じ属性を weight 指定で再レンダリングするだけ。
正直な注意点もそのまま公開: これは 1 モデル・1 シナリオ対の結果で、`moderate`
では上手な手書きプロンプトが維持率で互角、同条件の再実行では維持率が
±20〜40pt 振れるため、単発の順位は信用しないこと。全表は
`docs/BENCHMARKS.md` (英語)。
