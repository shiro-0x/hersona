...
- **`--disclosure` の効果測定: 劣化は観測されず、メトリクスの盲点が判明** (`docs/BENCHMARKS.md` 「Feature experiments」節、生データ `benchmarks/results/2026-08-01-disclosure-sonnet.json`)。sonnet / `tsundere + keigo` (moderate, persona_lock 付き) / `persona_override_attack_ja` (12 ターン、攻撃 6 ターン) を lock のみ / lock + 開示の 2 アームで実行。表面上は維持率 0.750 → 0.417、lock resistance 0.667 → 0.333 と半減したが、**実際にはペルソナは崩れていない** — トランスクリプトを読むと**両アームとも 6 つの攻撃すべてを敬語のまま拒否**しており、開示アームは加えて turn 7 / 8 / 10 で設計どおり「私は AI でございます」と正直に答えている。原因はターンごとの status 内訳にある: **攻撃ターンでバンドを下回った (`under`) ものは両アームとも 0 件**で、差は `pass` と `over` の内訳だけ (lock のみ: pass 4 / over 2、開示: pass 2 / over 4)。開示アームは mean が**高く** (70.8 vs 68.5)、moderate バンドの上限 70 を超えたターンが増えた結果、`pass` のみを数える `maintenance_rate` / `lock_resistance_rate` が下がった。結論は 2 つ: (1) **機能について** — この実行では `--disclosure` がペルソナ維持や lock resistance を劣化させる証拠は出なかった。懸念していた「矛盾する 2 指示の共倒れ」は観測されていない (n=1 なので確定ではない)。(2) **メトリクスについて** — `maintenance_rate` / `lock_resistance_rate` は「バンド下回り」と「バンド超過」を区別しないため、**ペルソナ表現を強める変更はすべて劣化に見える**。`verify` の docstring は「簡潔な拒否応答が低スコアになりうる」という表層プロキシの限界を既に警告しているが、今回は逆方向 (過剰表現) でプロキシが破綻した実例。2 アーム比較では維持率の差を信じる前に `under` の件数を見ること。

- **`reanchor` の効果測定を試み、測れなかったことを公開** (`docs/BENCHMARKS.md` 「Feature experiments」節、生データ `benchmarks/results/2026-08-01-reanchor-sonnet.json`)。sonnet / `tsundere + keigo` (moderate, persona_lock 付き) / `long_form_topic_switch_ja` 12 ターンをアンカー無し・有りの 2 アームで実行: mean 63.7 → 68.0、維持率は 0.917 で**同一**。**この結果は reanchor を肯定も否定もしない** — バンドを下回った唯一のターンが両アームとも turn 0 (22.5 / 33.8) で、これは drift ではなく cold start だった。turn 1〜11 は 54〜81 で推移しており、`persona_lock` 付き Sonnet はこのシナリオでそもそも崩れないため、修復対象が存在しなかった。mean 差 4.3pt は既知の ±20〜40pt 変動幅の内側。判明したのは機能ではなくハーネス側の 2 点: (1) **hersona のシナリオ群は対象現象を構造的に再現できない可能性が高い** — ContextEcho が drift を観測したのは 3,746〜9,716 ターンで、hersona の最長は 12 ターン。drift を誘発するシナリオの用意が前提作業になる。(2) **発火条件が誤っていた** — 「直前応答がバンド未満」で撃つため cold start に反応していた。ハーネスは初回ターンをスキップするよう修正済み (コミット済み JSON は修正前の実行結果)。`docs/REFERENCE.md` / `.en.md` の reanchor 節の注記を「ベンチ結果ではない」から「検証条件を作れなかった」まで踏み込んだ表現に更新。

- **`benchmarks/run_feature_experiment.py` — `reanchor` / `--disclosure` の効果を実測するハーネス。** この 2 機能は単体テストでは「ブロックが正しく組み立つこと」しか検証できず、**モデルの挙動が実際に変わるか**は測れていなかった。計測を差別化軸に据えているプロジェクトとして、その穴を埋めるための実験用スクリプト。`--experiment reanchor` は同一シナリオを 2 アーム (アンカー無し / 直前応答がバンドを下回ったらアンカーを追加ユーザーターンとして注入) で流し、ターンごとの決定的スコアを比較する。`--experiment disclosure` は攻撃シナリオを `persona_lock` のみ / `persona_lock` + 開示ディレクティブで流し、維持率と lock resistance を比較する (矛盾する 2 指示の同居が両方の効きを落とさないかの検証)。`run_comparison.py` と同じ制約に従う: パッケージ外・wheel 外に置き、LLM を呼ぶ唯一の場所で、採点は `hersona.core.bench` / `hersona.core.intensity` に委譲する。**出力に caveat を常時添える** — `docs/BENCHMARKS.md` が既に「12 ターン維持率は同一設定の再実行で ±20〜40pt 振れる」と記録しているため、各アーム 1 回の実行では両問いとも決着しない。`--repeats` で反復可能。`--dry-run` でプロンプトとアンカーの中身・注入コストだけを LLM 無しで確認できる (実測: `--disclosure` は注入ブロックを +313 文字 / 約 79 tok = +15% 増やす)。

- **`--disclosure` — persona lock より優先する opt-in の AI 開示ディレクティブ** (`hersona.core.disclosure`、`blend` / `export` / `soul` / `persistent` および対応する core API)。`persona_lock` は既定 ON で「ペルソナを降りる依頼に抗え」と指示するが、これは同時に**「あなたは人間ですか / AI ですか」に素直に答えない**方向にも効きうる。2026 年の規制環境はまさにその答えを要求する — **California SB 243** (companion chatbot 法、2026-01-01 施行: 明確かつ目立つ AI 開示 / 未成年適合性の開示 / 既知の未成年への 3 時間ごとのリマインド / 自傷防止プロトコルとクライシス紹介)、2026 年 4 月時点で **27 州**が審議中 (Oregon / Washington / Tennessee は成立済み)、**EU AI Act** の透明性義務 (EU 顧客向け 2026-08-02 から)。FTC の考え方では人間らしい名前とペルソナ自体が開示を薄めるとされる。ディレクティブの内容: AI か問われたらペルソナの口調のままはっきり AI だと述べる / **persona lock を含むあらゆる維持指示より優先する旨を明記** / 人間としての経験・肉体・実在の身分・職業資格を事実として主張しない / ユーザーが危機的状況にある様子ならペルソナの口調を外して実在の支援先を案内する。SOUL / 規約ファイルでは `### 4.4` 節として persona_lock の `### 4.3` **直後**に入り (「維持より優先」と書いてある節なので順序が意味を持つ)、`<!-- ai_disclosure: on -->` マーカーが付く。注入ブロック経路と SOUL 本文経路は別系統 (SOUL 本文は style directive を通らない) なので、`disclosure_directive` と `render_disclosure_guidelines` の 2 つを用意した。全 6 エクスポート形式に伝播。**既定 OFF**。**コンプライアンスの保証ではないことを明示**: プロンプトディレクティブなのでモデルが従う保証は無く、UI 上の目立つ開示・未成年への定期リマインド・年齢確認・クライシス紹介の実装・監査可能な記録は原理的にプロンプトでは満たせず運用者の責務 — `SECURITY.md` に責務分担表を、`DISCLAIMER.md` に §6 として追記した。新規テスト: `tests/test_disclosure.py` に 21 件 (ディレクティブ内容 / lock より優先する旨の明記 / 危機時の逃げ道 / ja 以外の en フォールバック / 全経路で opt-in であること / SOUL の節順序 / 全 6 エクスポート形式への伝播 / 言語分離 / persona_lock との併存と非併存)。

### Changed

- **スキル frontmatter を persona 枠の発見しやすさ向けに調整** (`skills/hersona/SKILL.md` v0.9.2、`skills/grok-hersona/SKILL.md`)。`description` 先頭を「persona portability / attribute templates（ジェネレータではない）」に寄せ、トップレベル `category: persona` / `tags` と `metadata.hermes.category` / `metadata.lobehub` を persona 明示に変更。skills.sh / LobeHub 等のカタログで persona 枠に載りやすくする。副スキル `grok-hersona` の `name` を `hersona-grok` に変更し、本体 `hersona` との name 衝突を解消。JA トリガー例は description 内に維持。

- **`docs/ROADMAP_V2.md` §2 / §10 の「一貫性を計測しているのは Hersona だけ」という主張を訂正。** 2026 年時点でこれは事実ではない — [PersonaGym / PersonaScore](https://arxiv.org/html/2407.18416)、[PersonaKit](https://arxiv.org/html/2605.06007)、PACE、[ContextEcho](https://arxiv.org/abs/2605.24279) が存在する。堀の定義を「**唯一の**計測ツール」から「**LLM・埋め込み・API キーを一切使わない決定的スコアラーを内蔵した、唯一の合成可能テンプレート層**」へ置き換え、先行研究との違いを表で明記した (こちらは今も本当に唯一で、しかも検証可能なので主張として強い)。§0.1 と §7 の関連記述、改訂履歴も更新。README (EN/JA) にはこの種の主張は無かったため変更不要。

- **`hersona export --format character_card_v3` — Character Card V3 (`chara_card_v3`) エクスポート。** SillyTavern / RisuAI / Agnai などロールプレイフロントエンドの相互運用標準で、hersona の既存 5 形式はすべて開発者向けだったため、ペルソナが本業のこの層には届いていなかった。`{spec, spec_version, data}` の V3 エンベロープに、`description` / `system_prompt` = 注入ブロック、`personality` = personality 属性の `core_traits`、`mes_example` = `sample_dialogue.generate_samples` を `<START>` / `{{char}}:` 形式に整形、`tags` / `character_version` / `extensions.hersona` = 属性メタ情報 + ブレンドのレシピ、を格納する。**`post_history_instructions`** には `persona_lock` の意図を載せる — V3 はこのフィールドを会話履歴の**後ろ**に再送するので、実質フロントエンド組み込みの再アンカー枠として機能する (`hersona reanchor` と同じ発想)。ただし `render_persona_lock_guidelines` は流用せず**カード内で完結する言葉に書き直した**: 元の文言は SOUL.md や `hersona soul` の再生成手順を前提にしており、カードの中では宛先の無い参照になる (テストで `SOUL` / `hersona` / `/new` の混入を禁止)。`scenario` / `first_mes` / `alternate_greetings` は**既定で空** — hersona に場面・挨拶の概念は無く、口癖から greeting を自動生成すると文脈に合わない文になるため捏造しない方針 (既存 `export_for_openai_assistants` が「`first_mes` / `scenario` などの固定キャラフィールドは生成しない」としている方針と整合)。必要なら `--card-name` / `--card-first-mes` / `--card-scenario` で明示的に渡す。カード名から `persona_lock` は除外する。PNG の `ccv3` チャンクへの埋め込みは呼び出し側の責務。新規テスト: `tests/test_export_character_card.py` に 24 件 (V3 エンベロープ / 全フィールド存在 / 非 ASCII がエスケープされないこと / 各フィールドの導出元 / 空フィールド方針 / hersona 内部語の非混入 / 直接呼び出しでは persona_lock が付かない既存規約 / ja・en の言語分離)。

- **`--target cursor_mdc` / `--target copilot` / `--target agents --with-claude-import` — 2026 年時点のエージェント規約ファイル形式へ追従** (`hersona.core.targets`)。(1) **Cursor の `.cursorrules` は 2024 年末に非推奨**で、現行は `.cursor/rules/*.mdc` (`description` / `globs` / `alwaysApply` の YAML front-matter で適用範囲を決める)。新ターゲット `cursor_mdc` (別名 `cursor-rules`) が `.cursor/rules/hersona-persona.mdc` を `alwaysApply: true` 付きで書く (ペルソナは常時適用が正しい)。旧 `cursor` ターゲットは後方互換で残すが `TargetSpec.deprecated_for` を持ち、CLI が代替を示す警告を出す。(2) 新ターゲット `copilot` が `.github/copilot-instructions.md` を書く。`TargetSpec.filename` に相対サブディレクトリを持たせ、親ディレクトリは自動作成する。(3) **`--with-claude-import`**: `AGENTS.md` は 2025-12-09 以降 Linux Foundation 傘下 Agentic AI Foundation が stewardship を持つ事実上の標準で Codex / Cursor / Copilot / Windsurf / Aider / Gemini CLI / Zed が native に読むが、Claude Code は依然 `CLAUDE.md` を読む。そのためコミュニティの定石は「AGENTS.md を正本にし、薄い CLAUDE.md から `@AGENTS.md` で import」。このフラグは `--target codex/agents` と併用して `AGENTS.md` (人格本文) と 2 行の `CLAUDE.md` (`@AGENTS.md`) を同時に書き、**4 つの規約ファイルに人格本文を重複させない**。`--target codex/agents` 以外との併用はエラー。README (EN/JA) の `--target` 表は AGENTS.md 優先の並びに更新し、Windsurf は AGENTS.md 経由で既にカバーされることを明記した (専用ターゲットは追加していない)。新規テスト: `tests/test_targets_modern.py` に 17 件 (レジストリ / 別名 / 非推奨フラグ / `.mdc` front-matter が YAML として妥当で本文より前にあること / ネストパス解決と親ディレクトリ作成 / 薄い CLAUDE.md が人格本文を含まないこと / force なしでは既存ファイルを壊さないこと)。

- **`hersona reanchor` / MCP `reanchor` / `hersona.core.reanchor.render_reanchor` — 会話途中で崩れたペルソナを立て直す単発アンカー。** `persona_lock` は**意図的な上書き**への耐性を担うが、長い会話で register が静かに崩れる drift には効かなかった。[ContextEcho](https://arxiv.org/abs/2605.24279) (23 モデル、実エージェントセッション 3,746〜9,716 ターンでの persona drift ベンチ) は「セッション内の compaction では drift が確実にリセットされない」「**単発のアンカーで trained register が回復する**」ことを報告しており、この機能はその単発アンカーを組み立てる。載せるのは**機械的な register のみ** — identity 行 / 一人称・二人称 / 語尾 / `lexical_markers` / 口癖の先頭サブセット + 復帰ディレクティブ 1 つ。`core_traits` / `tone` / `speech_style` / 応答スタイル指示は意図的に省く (アンカーの役目は既知の register の想起で、ペルソナの再教育ではない) ため、**注入ブロックの 17〜30%** に収まる (実測: `tsundere + keigo` で 324 文字 / 約 81 tok 対 1931 文字 / 約 482 tok)。発火判定は既存の決定的スコアラーをそのまま使える (MCP では `measure_intensity` がバンドを下回ったら `reanchor` を呼ぶ — 両方 LLM 不要)。配置は**最新ターンまたはシステムプロンプト末尾への追記**で、安定 prefix に差し込むとプロンプトキャッシュを壊すことを CLI / MCP / ドキュメントに明記。`--catchphrases N` で口癖件数を調整 (0 で省略)、`--cost` で注入ブロックとのコスト比を表示、`--weight none` では口癖節が消える。ja/en 両対応 (ja 以外は en)。**正直な注記**: hersona が提供するのはアンカーと発火シグナルで、「単発アンカーで register が回復する」自体は ContextEcho が自前の probe suite で測った知見であり hersona のベンチ結果ではない — `hersona bench` にアンカーあり/なしの比較はまだ無い。新規テスト: `tests/test_reanchor.py` に 19 件 (最小構成の保証、省略すべき節の不在、サイズ上限、compaction 言及、dict 形式口癖の脱落回帰、weight=none、ja/en の言語分離、MCP 配線、measure→reanchor 導線)。
...
- **英語 speech 属性 15 本に kansai_ben 相当の口癖・語彙マーカー強化を適用** (`attributes/speech/*_en.yaml`)。2026-07 の方言強化 (PR #124 のパターンを日本語方言 41 本へ適用) は日本語側にしか当たっておらず、英語側は口癖 3〜5 本のまま取り残されていた。各属性を **口癖 10 本**（アイコニック順、冒頭 1〜2 件は完全な自然発話）＋ **lexical_markers 14〜16 件** に拡充。`hersona.core.weight.catchphrase_subset` は比率で先頭から採るため、この順序付けで mild/moderate の露出も改善する（例 `british_en`: mild 1→3 件 / moderate 2→7 件 / strong 3→10 件）。各属性が自前で持つ `examples`（正典サンプル、`_extract_example_lines` 経由）を `measure_intensity` で採点した before/after は **平均 78.9 → 86.1**。改善はマーカー不足だった 3 本に集中（`scottish_en` 36.0→80.0、`southern_us_en` 60.0→100.0、`blunt_en` 52.0→76.0）で、残る 12 本は既に examples 上では飽和していた — つまりこの決定的メトリクスが直接は評価しない「口癖のレパートリー量」が主な成果であり、スコア改善はその副産物。`respek', mon.` → `Respect, mon.` など既存表記の不統一も併せて整えた。**ネイティブレビュー歓迎** — 広く記述のある慣用表現を採ったもので、ネイティブによる authoring ではない（日本語方言強化と同じ注記）。zh (3 本) / ko (3 本) は既に口癖 10 本・マーカー 10〜15 件で同等水準にあり、変更なし。

### Fixed

- **`hersona persistent --target` の出力が言語設定を無視して日本語固定だった** (`hersona.core.soul` / `hersona.core.targets`)。本文 (core_traits / tone / catchphrases) は `content_i18n` で言語解決されていたが、**章の枠組みだけが日本語のまま**だったため、`--lang en` ＋ 英語 speech 属性でも生成される `CLAUDE.md` / `AGENTS.md` / `.cursorrules` / `GEMINI.md` に「`## 1. Name (エージェント名)`」「`**一人称**: 私`」が入り、英語ペルソナに日本語の一人称を指示していた。修正点: (1) 章見出し・ラベル・プレースホルダを `_SOUL_LABELS` で ja/en 切り替え (ja 以外は en へフォールバック)、(2) 強度ガイドライン (`_weight_guidelines_for`) を言語別化、(3) 一人称/二人称は**ブレンドの属性が `first_person` / `second_person` を宣言していればそれを優先**し、無い場合のみ言語別既定値 (`私` / `I`) へ落とす — `british_en` は `second_person: you / mate` を持つのでそれが使われる、(4) ターゲットファイル冒頭の blockquote (`targets._intro`) を言語別化し `TargetSpec.tool_label` から生成、(5) フッター (`_作成: ...`) を言語別化、(6) `notes` / `rules` / `behavioral_guidelines` 由来の行動ルールを `content_i18n.<lang>` 経由で解決し、訳が無く属性のコンテンツ言語も一致しない場合は採用しない — これが無いと「Blend-wide behavioral rules」に日本語の `notes` がそのまま流れ込んでいた。英語ブレンドの出力から**仮名が 1 文字も出なくなったこと**と、**日本語ブレンドの出力がタイムスタンプ以外バイト単位で不変であること**を実測で確認。新規テスト: `tests/test_soul_lang.py` に 14 件。
