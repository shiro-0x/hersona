...
- `attributes/visual/{braids,hime_cut,bob_cut,messy_hair,ahoge,blonde,pink_hair,black_hair,red_hair,blue_hair,sharp_eyes,droopy_eyes,jitome,eyepatch,slender,muscular,freckles,tan,mole,scar}.yaml`: 20 new visual attributes (visual 5 → 25, catalog 201 → 221). Covers hairstyle (`braids`, `hime_cut`, `bob_cut`, `messy_hair`, `ahoge`), hair color (`blonde`, `pink_hair`, `black_hair`, `red_hair`, `blue_hair`), eyes (`sharp_eyes`, `droopy_eyes`, `jitome`, `eyepatch`), body type (`slender`, `muscular`), and facial marks (`freckles`, `tan`, `mole`, `scar`). Each follows the v1.6.0 `BASE=en + i18n.ja + content_i18n.en` format and includes required `image_prompt_tags` (ASCII). Some cross-references in the brief targeted attributes not yet implemented (`athlete`, `soldier`, `pirate_archetype`); those were removed in favor of in-catalog archetypes (`heroine`, `rival`, `mentor`).

### Changed

- `tests/catalog_counts.py`: `TOTAL_PUBLIC_ATTRIBUTES` 201 → 221, `visual` 5 → 25.
- README EN/JA, `CLAUDE.md`, `CONTRIBUTING.md`, `skills/hersona/SKILL.md`: synchronized public docs to the 221-attribute / visual 25 catalog state. The quickstart Visual bullet line now lists all 25 attribute names.
