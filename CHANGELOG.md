...
- Stale v1.8.0 test expectations updated: `tests/test_persistent.py` persona names now include the default-appended `persona_lock`; `tests/test_cli.py::test_no_color_disables_rich` reads the personality count from `tests/catalog_counts.py` instead of a hard-coded pre-v1.8.0 `42`.
