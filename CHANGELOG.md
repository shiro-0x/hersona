...
- `scripts/release_check.py` + `docs/RELEASE_CHECKLIST.md`: a pre-tag release dry-run script that runs every mechanical CI gate locally in one command (ruff, `validate.py`, `build_site.py --check`, `check_readme_counts.py`, `gen_checksums.py --check`, and optionally the full `pytest` suite), plus a manual checklist for the steps that aren't (and shouldn't be) automated — bumping `pyproject.toml`, moving the CHANGELOG heading, tagging, watching the publish workflow, and the manual GitHub "About" field update. Written in response to an external review noting "rough release operations" (the project's own semver-rollback history, recorded honestly rather than hidden, motivated catching this class of mistake earlier next time).
