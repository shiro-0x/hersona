...
hersona のフルリファレンス: 使いどころの判断、CLI 全コマンド、Hermes skill の
レシピ集、use case、ペルソナパック、export 形式、属性カタログ、YAML スキーマ。
[README](../README.ja.md) には要点のみを置き、詳細はすべてここに集約している。

- [hersona の使いどころ（と使わないところ）](#hersona-の使いどころと使わないところ)
- [CLI リファレンス](#cli-リファレンス)
- [Hermes Agent skill — レシピ集](#hermes-agent-skill--レシピ集)
- [プロ向け Operating Mode / use case](#プロ向け-operating-mode--use-case)
- [ペルソナパック](#ペルソナパック)
- [OpenAI Assistants / LangChain へ export](#openai-assistants--langchain-へ-export)
- [MCP サーバー詳細](#mcp-サーバー詳細)
- [他の LLM で使う](#他の-llm-で使う)
- [ガイド](#ガイド)
- [データ形式](#データ形式)
- [カタログ拡張史](#カタログ拡張史)

## hersona の使いどころ（と使わないところ）
...
こと」なら hersona は何も貢献しない。仕事の一部が「何者かであること」
(キャラクター、ブランドボイス、ロールプレイ相手) なら、それが hersona の管轄。
...
[`BENCHMARKS.md`](./BENCHMARKS.md)(英語)を参照。
...
hersona export tsundere keigo --format messages  # 他フレームワーク向けに export (json/messages/markdown)
...
hersona update --ref v1.9.0                    # ブランチ / タグ / コミット SHA を指定 (既定: main)
...
## Hermes Agent skill — レシピ集

tap 経由でインストールする (レジストリ審査は不要):
...
`hersona --help`、本ドキュメント、[`PUBLIC_API.md`](./PUBLIC_API.md) を優先。
...
初期 public use case（全 20 本、`PERSONA_PACKS_DESIGN.md` §6–§8 参照）:
...
同梱パック 14 本 (`PERSONA_PACKS_DESIGN.md` §6 参照):
...
| 静的テンプレ | conflict 検査済みブレンド + `hersona bench` で **維持率を測定可能** (人格上書き攻撃へのロック耐性を含む) |
...
上の表が正本、スキーマ追加は `PERSONA_PACKS_DESIGN.md` §6 に従うこと。
...
| ツール | 内容 |
|---|---|
| `list_attributes` / `show_attribute` | カタログの参照 |
| `blend` / `export` | ペルソナの合成と受け渡し (`export` は 5 形式すべて対応) |
...
`measure_intensity` と `bench_transcript` により、エージェント自身がループを
閉じられる: 応答を生成 → 採点 → ペルソナから外れていたら自己修正。CLI の
`hersona measure` / `hersona bench` と同じ決定的スコアラーを使う。
`install_persona` は設計上プレビュー専用 (MCP 呼び出しからファイルシステムへ
書き込まない)。実際のインストールは CLI の `hersona personas install <name>` で行う。
...
- [自己紹介](./guides/self-introduction.ja.md) — 公開向けルール・プライバシー・チェックリスト
- [English](./guides/self-introduction.md) · [一覧](./guides/README.md)
...
speech の変遷は 5 つの Phase に v1.5.0 波を加えた構成:
