...
2. **Benchmarkを最優先に前倒し** — 「人格が崩れない」ことを数値で示せるのがHersonaの堀。Phase 3→Phase 1へ移動し、最小版から出す。（2026-07-30 訂正: 「唯一の」は事実でなくなった。§2 参照）
...
## 2. 差別化：技術的な堀は「軽量・決定的な一貫性の計測」

> **2026-07-30 訂正。** 初版・改訂版はここを「**唯一の**ツール」と書いていたが、
> それは 2026 年時点で**事実ではない**。ペルソナ一貫性の評価には少なくとも次がある:
>
> | 先行研究・ツール | 何をするか | Hersona との違い |
> |---|---|---|
> | [PersonaGym / PersonaScore](https://arxiv.org/html/2407.18416) | persona agent の評価フレームワークと人間整合の自動指標 | LLM judge 依存。Hersona は決定的採点 |
> | [PersonaKit](https://arxiv.org/html/2605.06007) | 対話エージェントのプロトタイピング & 一貫性評価プラットフォーム | プラットフォーム。Hersona はライブラリ + CLI |
> | PACE | ペルソナ声・話題逸脱を検出して介入 | 介入まで含む。Hersona は計測と再アンカーを分離 |
> | [ContextEcho](https://arxiv.org/abs/2605.24279) | 23 モデル × 実セッション 3,746〜9,716 ターンの persona drift ベンチ | 大規模実測。Hersona は手元で再現できる軽量版 |
>
> 主張を正直な形に置き換える。**「誰も計測していない」ではなく「LLM・埋め込み・
> API キーを一切使わない決定的スコアラーを内蔵した、唯一の合成可能テンプレート層
> である」**。こちらは今も本当に唯一で、しかも検証可能なので主張として強い。

Hersonaの他OSSに対する優位は、機能の多さでも「計測している唯一性」でもなく、
**合成可能な属性テンプレート層に、依存ゼロ・決定的・再現可能な一貫性スコアラーが
同梱されている**こと。埋め込みも LLM judge も API キーも要らないので、CI に置ける。
...
- **Hacker News**: 技術記事3本を弾に
  - なぜ人格は崩れるのか（Benchmark実データ + ContextEcho 等の先行研究との突き合わせ）
...
その中で **「評価（Benchmark）」を、依存ゼロ・決定的・手元で再現可能な形で同梱しているのは Hersona だけ**（先行研究の一覧と主張の訂正は §2）。ここを押さえることで、Hersonaは単なるプロンプト集ではなく、AIエージェント時代の人格インフラとして独自のポジションを確立する。
...
- 2026-07-30: §2 / §10 の「一貫性を計測しているのは Hersona だけ」という主張を訂正。
  2026 年時点では PersonaGym / PersonaKit / PACE / ContextEcho が存在するため、堀を
  「唯一の計測ツール」から「依存ゼロ・決定的・再現可能なスコアラーを同梱した唯一の
  合成可能テンプレート層」へ置き換えた。あわせて ContextEcho の知見
  （compaction では drift がリセットされない / 単発アンカーで register が回復する）を
  `hersona reanchor` として実装。
